...
class HourglassNet(nn.Module):
    ...
    def __init__(self, block, head, depth, num_stacks, num_blocks, num_classes, dropout_rate=0.5):
    ...
    def forward(self, x):
        ...
        x = self.maxpool(x)
        x = self.layer2(x)
        x = self.layer3(x) # B x 256 x 128 x 128

        for i in range(self.num_stacks):
            y = self.hg[i](x) # y: B x 256 x 128 x 128
            y = self.res[i](y) # y: B x 256 x 128 x 128
            y = self.fc[i](y) # y: B x 256 x 128 x 128

            # # Added a dropout layer on top of original model
            # y = self.drop_layer(y)

            score = self.score[i](y) # score: B x 5 x 128 x 128, B x 5 x 128 x 128,
            ...
            if i < self.num_stacks - 1:
                fc_ = self.fc_[i](y) # B x 256 x 128 x 128
                score_ = self.score_[i](score) # B x 256 x 128 x 128
                x = x + fc_ + score_
