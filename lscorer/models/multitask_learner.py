...
class MultitaskHead(nn.Module):
    def __init__(self, input_channels, num_class):
        ...
        heads = []
        for output_channels in sum(configs.model.head_size, []):
            heads.append(
                nn.Sequential(
        ...
        assert num_class == sum(sum(configs.model.head_size, []))
...
class MultitaskLearner(nn.Module):
    def __init__(self, backbone):
        ...
        head_size = configs.model.head_size
        self.num_class = sum(sum(head_size, []))
        self.head_off = np.cumsum([sum(h) for h in head_size])

    def forward(self, input_dict):
        image = input_dict["image"]
        outputs, feature = self.backbone(image) # outputs:[B x 5 x 128 x 128, B x 5 x 128 x 128], feature: 6 x 256 x 128 x 128
        ...
        # switch to CNHW
        for task in ["jmap"]:
            T[task] = T[task].permute(1, 0, 2, 3) # 1 x 6 x 128 x 128
        for task in ["joff"]:
            T[task] = T[task].permute(1, 2, 0, 3, 4) # 1 x 2 x 6 x 128 x 128

        offset = self.head_off
        loss_weight = configs.model.loss_weight
        losses = []
